...
CTRNN::CTRNN(int newsize, double wmax, double bmax, double minP, double maxP)
{
	SetCircuitSize(newsize,wmax,bmax,minP,maxP);
#ifdef FAST_SIGMOID
  InitSigmoidTable();
#endif
}
...
void CTRNN::SetCircuitSize(int newsize, double wmax, double bmax, double minP, double maxP)
{
	size = newsize;
  ...
  amp = 0.0;
  ampGain = 1.0;
  minPeriod = minP; // in steps
  maxPeriod = maxP;  // in steps
  ...
  {
    period[i] = UniformRandomInteger(minPeriod,maxPeriod);
...
void CTRNN::Flux(double performance, RandomState &rs)
{
  // Cap performance [0,1]
  performance = performance > 1.0 ? 1.0 : performance;
  // Transform reward to amplitude
  amp = ampGain*pow(1-performance,2);
  ...
  for (int i = 1; i <= fluxsize; i++)
  {
      if (periodcount[i]==0) // Generate new period from distribution
      {
        ...
        period[i] = rs.UniformRandomInteger(5000,50000); //(2000,4000)
        periodcount[i] = period[i];
        flux[i] = 0.0;
      }
      else
...
void CTRNN::Learn(double reward)
{
  // Cap reward [-1,1]
  reward = reward > 1.0 ? 1.0 : reward;
  reward = reward < -1.0 ? -1.0 : reward;

  // Recalculate weight and bias centers based on
  for (int i = 1; i <= size; i++) {
    for (int j = 1; j <= size; j++) {
      weightcenters[j][i] += learnrate * (weights[j][i] - weightcenters[j][i]) * reward;
      // Cap weights and biases to max
      if (weightcenters[j][i] > weightmax)
        weightcenters[j][i] = weightmax;
      if (weightcenters[j][i] < -weightmax)
        weightcenters[j][i] = -weightmax;
    }
    biascenters[i] += learnrate * (biases[i] - biascenters[i]) * reward;
    if (biascenters[i] > biasmax)
      biascenters[i] = biasmax;
    if (biascenters[i] < -biasmax)
      biascenters[i] = -biasmax;
  }
}
...
istream& operator>>(istream& is, CTRNN& c)
{
	...
	int size;
	is >> size;
	c.SetCircuitSize(size,16,16,0,0);
